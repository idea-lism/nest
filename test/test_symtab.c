...
TEST(test_intern_multiple) {
  ...
  assert(strcmp(symtab_get(&st, 0), "alpha") == 0);
  assert(strcmp(symtab_get(&st, 1), "beta") == 0);
  assert(strcmp(symtab_get(&st, 2), "gamma") == 0);
  symtab_free(&st);
}
...
TEST(test_find_empty) {
  Symtab st = {0};
  symtab_init(&st, 0);
  assert(symtab_find(&st, "anything") == -1);
  symtab_free(&st);
}
...
TEST(test_intern_empty_string) {
  ...
  assert(id == 0);
  assert(strcmp(symtab_get(&st, 0), "") == 0);
...
TEST(test_prefixed_keys) {
  ...
  assert(t3 == 2);
  assert(t4 == 0);
  assert(strcmp(symtab_get(&st, 1), "scope:main") == 0);
  symtab_free(&st);
}
...
TEST(test_many_entries) {
  ...
  for (int32_t i = 0; i < 200; i++) {
    snprintf(buf, sizeof(buf), "sym_%d", i);
    assert(symtab_find(&st, buf) == i);
    assert(strcmp(symtab_get(&st, i), buf) == 0);
  }
  symtab_free(&st);
}
...
TEST(test_get_stable_after_growth) {
  ...
  assert(strcmp(symtab_get(&st, 0), "first") == 0);
  symtab_free(&st);
}
...
TEST(test_start_num) {
  ...
  assert(a == 10);
  assert(b == 11);
  assert(symtab_find(&st, "alpha") == 10);
  assert(symtab_find(&st, "beta") == 11);
  assert(symtab_find(&st, "nope") == -1);
  assert(strcmp(symtab_get(&st, 10), "alpha") == 0);
  assert(strcmp(symtab_get(&st, 11), "beta") == 0);
  assert(symtab_count(&st) == 2);
  symtab_free(&st);
}
