...
%tc = tt_current(tt)
%col = i64 0
%peg_table = tt_alloc_memoize_table(%tc, sizeof_col, slots_offset_in_i32, slots_size)
%tc->value = %peg_table
%tokens = tc->tokens
...
## Syntax-error best-try reporting

Generated constants per scope:
- `sizeof_col = bits_bucket_size * 8 + aligned_slots_size(slots_size) * 4`.
- `slots_offset_in_i32 = bits_bucket_size * 2` (i32 offset where physical slots start in a memoize column).
- `slots_size = ScopeClosure.slots_size`.
- These values are passed to `tt_alloc_memoize_table()` so they are stored in the `TokenChunk` for `tt_collect_parse_errors`.

Generate a callback function `{prefix}_parse_error_collect(void* parse_errors, int64_t* scope_ids, size_t scope_ids_size, uint64_t* col, int64_t slot_index, int64_t cp_offset, int64_t cp_size)` to be passed to `tt_collect_parse_errors` for diagnostics.
- If there are parse errors, vpa can call `tt_collect_parse_errors`.
- `{prefix}_parse_error_collect` can build syntax message and append to the `parse_errors` (see [vpa](vpa.md) for the struct details).
  - each message is in the form of either:
    - when there is best try, then message is `"{scope_chain}: syntax error after {rule_name}"`
       - `rule_name` is determined by the `col` and `slot_index`: switch `slot_index` and detect slot bit and find out the rule name.
    - when there is no best try, `col` is set to NULL, then message is `"{scope_chain}: syntax error"`
  - `scope_chain` is in the form of `[... scope2 in scope1 in main]`, from child to parent, generated by the scope_ids array.

This best-try message is diagnostic-only. It does not change parse success, memoization semantics, or loader behavior.
...
When parse succeeds, VPA will return a PegRef for `main` rule. PEG failure in a nested scope must not stop lexing; VPA records the syntax error and continues with the parent scope (see [vpa](vpa.md#error-reporting)).
...
- must not hard-coded field offset in IR, use `__builtin_offsetof` for field offset.
