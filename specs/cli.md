...
- environment variables
  - `CC` — Clang compiler to use for compiling runtime to LLVM IR. Falls back to `clang` in PATH. See [rt_ir](rt_ir.md).
...
- see [rt_ir](rt_ir.md) for how the `.ll` is produced.
...
1. call `common_header_gen(header_writer)` (defined in src/common_header_gen.h and src/common_header_gen.c), which:
   - uses build/nest_rt.inc as generated by [amalgamate](rt_ir.md) to start the runtime header
