...
```c
struct Table${scope_name} {
  Col${scope_name} cols[{token_size}];
};
...
{scope_name}$foo:
  ...
  ...
  ; defined tree generated by peg_ir with nested inline gen
  { if unit.tag_offset >= 0}
    %tag_bits |= {1 << (scoped_unit.tag_offset + scoped_rule.tag_bit_offset)}
  { end }
  ...

  { if rule has tag bits }
    %bits = gep(table->cols[%col].bits);
...
- Read memoize table slot
  - `table->cols[%col].slots[%slot_index]`
- Write memoize table slot
  - `table->cols[%col].slots[%slot_index] = val`
...
bool {prefix}_has_next(PegLink l) {
  int32_t* col = (int32_t*)l.elem.tc->value + {col_size_in_i32} * l.elem.col;
  int32_t lhs_slot = col[l.elem.row];
  if (lhs_slot < 0) {
    return false;
  }
  if (col.rhs_row >= 0) { // interlaced, also need to check rhs
    int32_t* rhs_col = col + {col_size_in_i32} * lhs_slot;
