...
- if it is a branch rule, store the matched scope_rule_id
...
- `define internal parse_{scope_name}`: callable from the IR generated by [VPA](vpa.md) because they live in the same IRWriter.

# Error handling in generated code

TODO: implement cut operator `^`, when no token can match after cut, report cut error.
